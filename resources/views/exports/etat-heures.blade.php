...
<div style="text-align:center; margin-top:25px; margin-bottom:35px;">
    <h1 style="font-size:16pt; font-weight:bold; color:#111827; text-transform:uppercase; letter-spacing:1px; margin:0; padding-bottom:8px;">ÉTAT GLOBAL DES HEURES PÉDAGOGIQUES</h1>
    <div style="width:80px; height:2px; background-color:#5a0661; margin:0 auto 8px auto;"></div>
    <div style="font-size:9pt; color:#4b5563; font-style:italic;">@if($filtres){{ $filtres }}@else Récapitulatif consolidé des volumes horaires par enseignant @endif</div>
</div>
...
<table style="width:100%; border-collapse:collapse; margin-bottom:35px; border:1px solid #d1d5db;">
    <tr style="background-color:#f9fafb;">
        ...
        <td style="padding:12px 15px; width:50%;">
            ...
            <div style="font-size:10.5pt; font-weight:bold; color:#166534; text-align:right;">{{ number_format((float)$summary['volume_total_heures'],2,',',' ') }} h</div>
        </td>
    </tr>
</table>
...
<table style="width:100%; border-collapse:collapse; table-layout:fixed; margin-bottom:40px;">
    <thead>
        <tr style="background-color:#5a0661; color:#ffffff;">
            <th style="padding:10px 8px; text-align:center; font-size:8pt; font-weight:bold; width:5%; border:1px solid #5a0661;">N°</th>
            <th style="padding:10px 12px; text-align:left; font-size:8pt; font-weight:bold; width:28%; border:1px solid #5a0661;">Nom</th>
            <th style="padding:10px 12px; text-align:left; font-size:8pt; font-weight:bold; width:27%; border:1px solid #5a0661;">Prénom</th>
            <th style="padding:10px 12px; text-align:center; font-size:8pt; font-weight:bold; width:18%; border:1px solid #5a0661;">Nb. Activités</th>
            <th style="padding:10px 12px; text-align:right; font-size:8pt; font-weight:bold; width:22%; border:1px solid #5a0661;">Volume horaire (h)</th>
        </tr>
    </thead>
    <tbody>
        @foreach($rows as $row)
        <tr style="background-color:{{ $loop->even ? '#fdfaff' : '#ffffff' }}; border-bottom:1px solid #e5e7eb;">
            ...
            <td style="padding:9px 12px; text-align:right; color:#111827; font-weight:bold; border-right:1px solid #e5e7eb;">{{ number_format((float)$row[4],2,',',' ') }} <span style="font-size:7pt; font-weight:normal; color:#9ca3af;">h</span></td>
        </tr>
        @endforeach
    </tbody>
    <tfoot>
...
<table style="width:100%; border-collapse:collapse; margin-top:10px;">
    <tr>
        ...
        <td style="background-color:#f9fafb; border:1px solid #e5e7eb; border-left:none; padding:12px 15px; font-size:8pt; color:#374151; line-height:1.5;">
            <strong>Observations :</strong> Cet état récapitulatif présente les volumes horaires déclarés et validés par les enseignants pour la période sélectionnée. Ces données servent de base au calcul des paiements.
        </td>
    </tr>
</table>
